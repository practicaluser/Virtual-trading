...
def process_pending_limit_orders():
    ...
    ).select_related('user', 'stock') # DB 쿼리 최적화
    ...
    for order in pending_orders:
        try:
            ...
            if current_price is None:
                logger.warning(f"{order.stock.stock_name} (주문 ID: {order.id})의 현재가를 가져올 수 없습니다. 건너<0xEB><0x9B><0x81>니다.")
                continue

            # 2. 주문 유형에 따라 체결 조건 확인
            should_execute = False
            execution_price = order.limit_price # 지정가 또는 더 유리한 가격으로 체결 (일반적으로 지정가 사용)
            if order.order_type == Order.OrderType.BUY and current_price <= order.limit_price:
                should_execute = True
                # 실제 체결가는 지정가 또는 현재가 중 유리한 쪽으로 할 수 있으나, 여기서는 지정가로 통일
            elif order.order_type == Order.OrderType.SELL and current_price >= order.limit_price:
                should_execute = True
                # 실제 체결가는 지정가 또는 현재가 중 유리한 쪽으로 할 수 있으나, 여기서는 지정가로 통일

            # 3. 조건 충족 시 체결 실행
            if should_execute:
                logger.info(f"주문 ID 실행: {order.id} ({order.order_type} {order.stock.stock_name} @ {execution_price} vs 현재가 {current_price})")
                try:
                    ...
                    with db_transaction.atomic():
                        ...
                        if order.order_type == Order.OrderType.BUY:
                            ...
                            if user.cash_balance < total_cost:
                                raise ValueError("체결 시점 예수금 부족.") # 주문 실패 처리
                            ...
                            total_cost_prev = portfolio.average_purchase_price * portfolio.total_quantity
                            total_quantity_new = portfolio.total_quantity + order.quantity
                            # 평단가 재계산
                            portfolio.average_purchase_price = (total_cost_prev + total_cost) / total_quantity_new
                            ...
                            try:
                                ...
                                if portfolio.total_quantity < order.quantity:
                                    raise ValueError("체결 시점 보유 수량 부족.") # 주문 실패 처리
                            except Portfolio.DoesNotExist:
                                 raise ValueError("체결 시점 포트폴리오 없음.") # 주문 실패 처리
                ...
                except (ValueError, Portfolio.DoesNotExist) as exec_error: # 체결 시점 유효성 검사 오류 처리
                     logger.warning(f"주문 ID {order.id} 체결 실패 (유효성 검사 오류): {exec_error}")
    ...
    result_message = f"미체결 주문 처리 완료. 체결: {executed_count}, 실패: {failed_count}"
