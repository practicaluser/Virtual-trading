...
class TradingAPITests(APITestCase):
    ...
    def setUp(self):
        ...
        # 테스트용 주식 생성
        self.stock_samsung = Stock.objects.create(stock_code='005930', stock_name='삼성전자', market_type='KOSPI')
        self.stock_sk = Stock.objects.create(stock_code='000660', stock_name='SK하이닉스', market_type='KOSPI')
        self.stock_naver = Stock.objects.create(stock_code='035420', stock_name='NAVER', market_type='KOSPI')

        # 기본 사용자의 초기 포트폴리오 설정 (삼성전자 10주 보유)
        self.portfolio_samsung = Portfolio.objects.create(
            user=self.user, stock=self.stock_samsung, total_quantity=10,
            average_purchase_price=Decimal('70000.00')
        )
        # 다른 사용자의 포트폴리오 (테스트 격리 확인용)
        Portfolio.objects.create(
            user=self.other_user, stock=self.stock_naver, total_quantity=5,
        ...
        self.pending_orders_url = reverse('pending-order-list') # GET /api/trading/orders/pending/
    ...
    def test_buy_new_stock_market_success(self, mock_get_price):
        ...
        self.assertTrue(Transaction.objects.filter(order_id=response.data['id']).exists())
    ...
    def test_buy_existing_stock_market_success(self, mock_get_price):
        ...
        data = {"stock": "005930", "order_type": "BUY", "quantity": 10, "price_type": "MARKET"}
        response = self.client.post(self.order_url, data)
        ...
        self.assertEqual(self.portfolio_samsung.average_purchase_price, Decimal('65000.00'))
    ...
    def test_sell_partial_stock_market_success(self, mock_get_price):
        ...
        self.assertEqual(self.portfolio_samsung.average_purchase_price, Decimal('70000.00')) # 평단가 불변
    ...
    def test_sell_all_stock_market_success(self, mock_get_price):
        ...
        data = {"stock": "005930", "order_type": "SELL", "quantity": 10, "price_type": "MARKET"}
        response = self.client.post(self.order_url, data)

        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(response.data['status'], Order.StatusType.COMPLETED)

        self.assertFalse(Portfolio.objects.filter(user=self.user, stock=self.stock_samsung).exists()) # 삭제 확인
        self.user.refresh_from_db()
    ...
    def test_buy_fail_market_insufficient_funds(self, mock_get_price):
        ...
        self.assertFalse(Portfolio.objects.filter(user=self.user, stock=self.stock_sk).exists())
    ...
    def test_sell_fail_market_insufficient_shares(self, mock_get_price):
        """[실패] 시장가 매도 - 보유 수량 부족"""
        mock_get_price.return_value = Decimal('80000.00')
        data = {"stock": "005930", "order_type": "SELL", "quantity": 11, "price_type": "MARKET"}
        response = self.client.post(self.order_url, data)
    ...
    def test_sell_fail_market_stock_not_owned(self, mock_get_price):
        ...
        mock_get_price.return_value = Decimal('150000.00')
        data = {"stock": "000660", "order_type": "SELL", "quantity": 1, "price_type": "MARKET"}
        response = self.client.post(self.order_url, data)
    ...
    def test_limit_buy_success_pending(self):
        ...
        data = { "stock": "000660", "order_type": "BUY", "quantity": 5, "price_type": "LIMIT", "limit_price": 100000.00 }
        response = self.client.post(self.order_url, data)
        ...
        self.assertEqual(self.user.cash_balance, Decimal('10000000.00'))
        self.assertFalse(Portfolio.objects.filter(user=self.user, stock=self.stock_sk).exists())
        self.assertFalse(Transaction.objects.filter(order_id=response.data['id']).exists())

    def test_limit_sell_success_pending(self):
        """[성공] 지정가 매도 주문 (PENDING 상태로 생성)"""
        data = { "stock": "005930", "order_type": "SELL", "quantity": 8, "price_type": "LIMIT", "limit_price": 90000.00 } # 10주 보유 중
        response = self.client.post(self.order_url, data)
    ...
    def test_limit_buy_fail_validation_insufficient_funds(self):
        ...
        data = { "stock": "005930", "order_type": "BUY", "quantity": 100, "price_type": "LIMIT", "limit_price": 100001.00 }
        response = self.client.post(self.order_url, data)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_limit_sell_fail_validation_insufficient_shares(self):
        ...
        data = { "stock": "005930", "order_type": "SELL", "quantity": 11, "price_type": "LIMIT", "limit_price": 90000.00 }
        response = self.client.post(self.order_url, data)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_limit_order_fail_missing_limit_price(self):
        ...
        data = { "stock": "005930", "order_type": "BUY", "quantity": 1, "price_type": "LIMIT" }
        response = self.client.post(self.order_url, data)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_buy_fail_market_api_connection_error(self, mock_get_price):
        ...
        data = {"stock": "000660", "order_type": "BUY", "quantity": 1, "price_type": "MARKET"}
        response = self.client.post(self.order_url, data)
        ...
        self.assertIsInstance(response.data, list)
        self.assertTrue(any("네이버 금융 서버 요청 실패" in str(e) for e in response.data))
    ...
    def test_get_order_list_includes_execution_details(self, mock_get_price):
        ...
        # 1. 주문 생성
        buy_res = self.client.post(self.order_url, {"stock": "000660", "order_type": "BUY", "quantity": 5, "price_type": "MARKET"})
        limit_res = self.client.post(self.order_url, {"stock": "005930", "order_type": "SELL", "quantity": 2, "price_type": "LIMIT", "limit_price": 80000.00})
        failed_order = Order.objects.create(user=self.user, stock=self.stock_naver, order_type="BUY", quantity=1, price_type="MARKET", status=Order.StatusType.FAILED)
        ...
        self.assertEqual(Decimal(completed_order_data['executed_price']), Decimal('160000.00')) 
        self.assertEqual(Decimal(completed_order_data['total_amount']), Decimal('800000.00')) 
    ...
    def test_get_portfolio_api_success_multiple_items(self, mock_get_price):
        def mock_price_logic(stock_code):
            ...
            if stock_code == '035420': return Decimal('260000.00')
            return Decimal('0.00')
        mock_get_price.side_effect = mock_price_logic

        Portfolio.objects.create(user=self.user, stock=self.stock_naver, total_quantity=2, average_purchase_price=Decimal('250000.00'))

        response = self.client.get(self.portfolio_url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 2)

        samsung_data = next(item for item in response.data if item['stock']['stock_code'] == '005930')
        naver_data = next(item for item in response.data if item['stock']['stock_code'] == '035420')
        ...
        self.assertAlmostEqual(Decimal(samsung_data['profit_loss_rate']), Decimal('14.285714'), places=6) 
        ...
        self.assertAlmostEqual(Decimal(naver_data['profit_loss_rate']), Decimal('4.0'), places=6) 
    ...
    def test_trading_apis_require_authentication(self):
        ...
        self.assertEqual(response_get_portfolio.status_code, status.HTTP_401_UNAUTHORIZED)
    ...
    def test_get_pending_order_list_success(self):
        ...
        # 1. 다양한 상태의 주문 생성
        pending_order1 = Order.objects.create(user=self.user, stock=self.stock_sk, order_type="BUY", quantity=1, price_type="LIMIT", limit_price=Decimal("100000"), status=Order.StatusType.PENDING)
        pending_order2 = Order.objects.create(user=self.user, stock=self.stock_naver, order_type="SELL", quantity=2, price_type="LIMIT", limit_price=Decimal("200000"), status=Order.StatusType.PENDING)
        Order.objects.create(user=self.user, stock=self.stock_samsung, order_type="BUY", quantity=1, price_type="MARKET", status=Order.StatusType.COMPLETED)
        Order.objects.create(user=self.other_user, stock=self.stock_samsung, order_type="BUY", quantity=1, price_type="LIMIT", limit_price=Decimal("70000"), status=Order.StatusType.PENDING)
    ...
    def test_cancel_pending_order_success(self):
        ...
        order_to_cancel = Order.objects.create(
            user=self.user, stock=self.stock_sk, order_type="BUY", quantity=1,
            price_type="LIMIT", limit_price=Decimal("100000"), status=Order.StatusType.PENDING
        )
        order_id = order_to_cancel.id
        cancel_url = reverse(self.cancel_order_url_template, kwargs={'order_id': order_id})
    ...
    def test_cancel_order_fail_not_pending(self):
        ...
        completed_order = Order.objects.create(
            user=self.user, stock=self.stock_samsung, order_type="BUY", quantity=1,
            price_type="MARKET", status=Order.StatusType.COMPLETED
        )
        Transaction.objects.create(user=self.user, stock=self.stock_samsung, order=completed_order, transaction_type="BUY", quantity=1, executed_price=Decimal("75000"))
    ...
    def test_cancel_order_fail_not_owner(self):
        ...
        other_user_order = Order.objects.create(
            user=self.other_user, stock=self.stock_sk, order_type="BUY", quantity=1,
            price_type="LIMIT", limit_price=Decimal("100000"), status=Order.StatusType.PENDING
        )
    ...
    def test_cancel_order_fail_not_exist(self):
        ...
        cancel_url = reverse(self.cancel_order_url_template, kwargs={'order_id': non_existent_order_id})
...
class TradingTaskTests(TestCase): # TestCase 상속
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            email='taskuser@example.com', nickname='taskuser', password='password123',
        ...
        self.portfolio_sk = Portfolio.objects.create(
            user=self.user, stock=self.stock_sk, total_quantity=10,
            average_purchase_price=Decimal('100000.00')
        )

    # --- Task: process_pending_limit_orders 테스트 ---

    @patch('trading.tasks.get_current_stock_price_for_trading') # Task 파일 내의 함수 Mocking
    def test_task_process_buy_order_executed(self, mock_get_price):
        ...
        limit_order = Order.objects.create(
            user=self.user, stock=self.stock_samsung, order_type="BUY", quantity=5,
            price_type="LIMIT", limit_price=Decimal("75000"), status=Order.StatusType.PENDING
        )
        ...
        self.assertEqual(limit_order.status, Order.StatusType.COMPLETED) # 상태 변경 확인
        ...
        # 포트폴리오 생성/업데이트 확인
        portfolio_samsung = Portfolio.objects.get(user=self.user, stock=self.stock_samsung)
    ...
    def test_task_process_sell_order_executed(self, mock_get_price):
        ...
        limit_order = Order.objects.create(
            user=self.user, stock=self.stock_sk, order_type="SELL", quantity=5,
            price_type="LIMIT", limit_price=Decimal("110000"), status=Order.StatusType.PENDING
        )
        ...
        self.assertEqual(self.portfolio_sk.average_purchase_price, Decimal("100000.00")) # 평단가 불변

    @patch('trading.tasks.get_current_stock_price_for_trading')
    def test_task_process_buy_order_not_executed(self, mock_get_price):
        """[성공] Task - 지정가 매수 주문 미체결 (현재가 > 지정가)"""
        limit_order = Order.objects.create(user=self.user, stock=self.stock_samsung, order_type="BUY", quantity=5, price_type="LIMIT", limit_price=Decimal("75000"), status=Order.StatusType.PENDING)
        ...
        self.assertFalse(Transaction.objects.filter(order=limit_order).exists()) # 거래 없음
        # 사용자 자산 불변 확인 (생략 가능)

    @patch('trading.tasks.get_current_stock_price_for_trading')
    def test_task_process_sell_order_not_executed(self, mock_get_price):
        """[성공] Task - 지정가 매도 주문 미체결 (현재가 < 지정가)"""
        limit_order = Order.objects.create(user=self.user, stock=self.stock_sk, order_type="SELL", quantity=5, price_type="LIMIT", limit_price=Decimal("110000"), status=Order.StatusType.PENDING)
    ...
    @patch('trading.tasks.get_current_stock_price_for_trading')
    def test_task_process_buy_order_failed_insufficient_funds_at_execution(self, mock_get_price):
        """[실패] Task - 지정가 매수 체결 시점 예수금 부족"""
        limit_order = Order.objects.create(user=self.user, stock=self.stock_samsung, order_type="BUY", quantity=15, price_type="LIMIT", limit_price=Decimal("75000"), status=Order.StatusType.PENDING) # 1,125,000원 필요
    ...
    @patch('trading.tasks.get_current_stock_price_for_trading')
    def test_task_process_sell_order_failed_insufficient_shares_at_execution(self, mock_get_price):
        """[실패] Task - 지정가 매도 체결 시점 보유 수량 부족"""
        limit_order = Order.objects.create(user=self.user, stock=self.stock_sk, order_type="SELL", quantity=15, price_type="LIMIT", limit_price=Decimal("110000"), status=Order.StatusType.PENDING) # 15주 매도 (10주 보유)
    ...
    def test_task_handles_api_error_gracefully(self, mock_get_price):
        ...
        pending_order1 = Order.objects.create(user=self.user, stock=self.stock_samsung, order_type="BUY", quantity=1, price_type="LIMIT", limit_price=Decimal("70000"), status=Order.StatusType.PENDING)
        pending_order2 = Order.objects.create(user=self.user, stock=self.stock_sk, order_type="BUY", quantity=1, price_type="LIMIT", limit_price=Decimal("100000"), status=Order.StatusType.PENDING)
        ...
        self.assertEqual(pending_order1.status, Order.StatusType.PENDING) # 에러 발생한 주문은 PENDING 유지
        self.assertEqual(pending_order2.status, Order.StatusType.COMPLETED) # 정상 조회된 주문은 체결됨
