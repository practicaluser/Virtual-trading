...
class Command(BaseCommand):
    ...
    def is_last_trading_day_of_month(self, date_to_check: date) -> bool:
        ...
        try:
            ...
            df = stock.get_market_ohlcv_by_date(fromdate=first_day_str, todate=last_day_str, ticker="005930")
            ...
            if is_last:
                ...
                logger.info(f"{date_to_check}은(는) {month}월의 마지막 거래일이 아닙니다 (실제 마지막 거래일: {last_trading_day}).")
    ...
    def handle(self, *args, **options):
        ...
        if not self.is_last_trading_day_of_month(today):
            ...
            message = f"{today}은(는) 마지막 거래일이 아니므로 스냅샷을 기록하지 않습니다."
                ...
                for item in portfolio_items:
                    try:
                        ...
                        if current_price is not None:
                            ...
                            logger.warning(f"경고: {user.email}의 {item.stock.stock_name} 현재가 조회 실패.")
